...
| **Implemented As** | (1) Added `MaxTrackFieldLength = 15` constant and validation in `ChdTocParser.ParseTextTracks`: TYPE/SUBTYPE fields > 15 chars cause the track entry to be skipped; PGTYPE/PGSUB fields > 15 chars likewise skip the entry. (2) Lowered `MaxKeyValueTextLength` from 64 KiB to 4 KiB — track metadata payloads > 4 KiB are rejected. (3) `ParseTextTracks` now trims trailing null padding and rejects entries with embedded null bytes (malformed payloads). (4) Lowered `MaxMetadataEntryBytes` in `ChdMetaData` from 1 MiB to 64 KiB — metadata entries with a `length` field > 64 KiB return `Chderrinvaliddata`. (5) Added 12 corpus tests: oversized TYPE/SUBTYPE/PGTYPE/PGSUB field (> 15 chars) → skipped; exact 15-char field → accepted; embedded null bytes → skipped; trailing null padding → tolerated; oversized payload > 4 KiB → rejected; oversized value in key-value dict → dropped; GDROM with oversized TYPE → skipped; `ReadMetaDataEntries` rejects entry at 65537 bytes. All 42 ChdTocParserTests and HeaderAndApiTests pass across net8.0/net9.0/net10.0. |
...
| **Implemented As** | Added `ReadAheadManager` private nested class inside `ChdFile`. Uses `ConcurrentDictionary<uint, byte[]>` as the L2 cache, `SemaphoreSlim` to cap concurrent decompressions at `lookAhead`, `ThreadLocal<ChdCodecState>` for thread-safe codec access, and `CancellationTokenSource` for clean shutdown. Background tasks use `ReadHunkConcurrent` for thread-safe decompression (no shared codec or stream position). `ReadHunk` checks the read-ahead cache (L2) before the LRU cache (L1). On L2 hit, the hunk is also seeded into the LRU cache. On successful decompression (including parent-referenced hunks), read-ahead is triggered for the next N hunks. Added `ConfigureReadAhead(int lookAhead)` (0/negative disables), `ReadAheadHunkCount` property, and `FlushReadAhead()` to clear stale entries after seeks. Both `Dispose` and `DisposeAsync` clean up the manager. Added 13 unit tests (`ReadAheadTests`): property defaults, configuration, correct data, sequential access benefit, flush, dispose, LRU seeding, boundary handling. Full suite 4614/4615 (pre-existing LaserDisc flaky test excluded). |
| **Estimated Time** | 6–8 hours |
